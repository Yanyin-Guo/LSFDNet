...
"""
edited by gyy in 2025/5/27
All the code is sourced from ultralytics/data/augment.py, 
and it has been modified to handle dual-modal image processing 
instead of single-modal image processing.
"""
...
class Mosaic(BaseMixTransform):
    ...
    def _mosaic3(self, labels):
        ...
        for i in range(3):
            ...
            an_img = labels_patch["an_img"]
            h, w = labels_patch.pop("resized_shape")

            # Place img in img3
            if i == 0:  # center
                img3 = np.full((s * 3, s * 3, img.shape[2]), 114, dtype=np.uint8)  # base image with 3 tiles
                an_img3 = np.full((s * 3, s * 3, an_img.shape[2]), 114, dtype=np.uint8)  # base image with 3 tiles
                h0, w0 = h, w
            ...
            an_img3[y1:y2, x1:x2] = an_img[y1 - padh :, x1 - padw :]
        ...
        final_labels["an_img"] = an_img3[-self.border[0] : self.border[0], -self.border[1] : self.border[1]]
        return final_labels
    ...
    def _mosaic4(self, labels):
        ...
        for i in range(4):
            ...
            if i == 0:  # top left
                ...
                an_img4 = np.full((s * 2, s * 2, an_img.shape[2]), 114, dtype=np.uint8)
            ...
            an_img4[y1a:y2a, x1a:x2a] = an_img[y1b:y2b, x1b:x2b]
            padw = x1a - x1b
            padh = y1a - y1b
    ...
    def _mosaic9(self, labels):
        ...
        for i in range(9):
            ...
            if i == 0:  # center
                ...
                an_img9 = np.full((s * 3, s * 3, an_img.shape[2]), 114, dtype=np.uint8)  # base image with 4 tiles
                h0, w0 = h, w
                c = s, s, s + w, s + h  # xmin, ymin, xmax, ymax (base) coordinates
            elif i == 1:  # top
                c = s, s - h, s + w, s
            ...
            an_img9[y1:y2, x1:x2] = an_img[y1 - padh :, x1 - padw :]  # img9[ymin:ymax, xmin:xmax]
            hp, wp = h, w  # height, width previous for next iteration
        ...
        final_labels["an_img"] = an_img9[-self.border[0] : self.border[0], -self.border[1] : self.border[1]]
        return final_labels
...
class MixUp(BaseMixTransform):
    ...
    def _mix_transform(self, labels):
        ...
        labels["an_img"] = (labels["an_img"] * r + labels2["an_img"] * (1 - r)).astype(np.uint8)
...
class RandomPerspective:
    ...
    def affine_transform(self, img, an_img, border):
        """
        ...
        if (border[0] != 0) or (border[1] != 0) or (M != np.eye(3)).any():  # image changed
            if self.perspective:
                ...
                an_img = cv2.warpPerspective(an_img, M, dsize=self.size, borderValue=(114, 114, 114)) 
            else:  # affine
                img = cv2.warpAffine(img, M[:2], dsize=self.size, borderValue=(114, 114, 114))
                an_img = cv2.warpAffine(an_img, M[:2], dsize=self.size, borderValue=(114, 114, 114))
        return img, an_img,  M, s
    ...
    def __call__(self, labels):
        ...
        img = labels["img"]
        an_img = labels["an_img"]
        cls = labels["cls"]
        instances = labels.pop("instances")
        # Make sure the coord formats are right
        instances.convert_bbox(format="xyxy")
        instances.denormalize(*img.shape[:2][::-1])
        #instances.denormalize(*an_img.shape[:2][::-1])

        border = labels.pop("mosaic_border", self.border)
        self.size = img.shape[1] + border[1] * 2, img.shape[0] + border[0] * 2  # w, h
        # M is affine matrix
        # Scale for func:`box_candidates`
        img, an_img, M, scale = self.affine_transform(img, an_img, border)
        #an_img, M, scale = self.affine_transform(an_img, border)
...
class RandomHSV:
    ...
    def __call__(self, labels):
        ...
        if self.hgain or self.sgain or self.vgain:
            ...
            an_hue, an_sat, an_val = cv2.split(cv2.cvtColor(an_img, cv2.COLOR_BGR2HSV))
            im_hsv = cv2.merge((cv2.LUT(hue, lut_hue), cv2.LUT(sat, lut_sat), cv2.LUT(val, lut_val)))
            an_hsv = cv2.merge((cv2.LUT(an_hue, lut_hue), cv2.LUT(an_sat, lut_sat), cv2.LUT(an_val, lut_val)))
            cv2.cvtColor(im_hsv, cv2.COLOR_HSV2BGR, dst=img)  # no return needed
            cv2.cvtColor(an_hsv, cv2.COLOR_HSV2BGR, dst=an_img)  # no return needed
        return labels
...
class RandomFlip:
    ...
    def __call__(self, labels):
        ...
        if self.direction == "vertical" and random.random() < self.p:
            img = np.flipud(img)
            an_img = np.flipud(an_img)
            instances.flipud(h)
        if self.direction == "horizontal" and random.random() < self.p:
            img = np.fliplr(img)
            an_img = np.fliplr(an_img)
            instances.fliplr(w)
            # For keypoints
            if self.flip_idx is not None and instances.keypoints is not None:
                instances.keypoints = np.ascontiguousarray(instances.keypoints[:, self.flip_idx, :])
        labels["img"] = np.ascontiguousarray(img)
        labels["an_img"] = np.ascontiguousarray(an_img)
...
class LetterBox:
    ...
    def __call__(self, labels=None, image=None):
        ...
        an_img = labels.get("an_img") if image is None else image
        ...
        if shape[::-1] != new_unpad:  # resize
            ...
            an_img = cv2.resize(an_img, new_unpad, interpolation=cv2.INTER_LINEAR)
        top, bottom = int(round(dh - 0.1)) if self.center else 0, int(round(dh + 0.1))
        left, right = int(round(dw - 0.1)) if self.center else 0, int(round(dw + 0.1))
        img = cv2.copyMakeBorder(
            img, top, bottom, left, right, cv2.BORDER_CONSTANT, value=(114, 114, 114)
        )  # add border
        an_img = cv2.copyMakeBorder(
            an_img, top, bottom, left, right, cv2.BORDER_CONSTANT, value=(114, 114, 114)
        )  # add border
...
class CopyPaste(BaseMixTransform):
    ...
    def _transform(self, labels1, labels2={}):
        ...
        an_img_new = np.zeros(an_img.shape, np.uint8)
        ...
        an_result = labels2.get("img", cv2.flip(an_img, 1))  # augment segments
        i = im_new.astype(bool)
        an_i = an_img_new.astype(bool)
        im[i] = result[i]
        an_img[an_i] = an_result[an_i]

        labels1["img"] = im
        labels1["an_img"] = an_img
        labels1["cls"] = cls
    ...
    def __call__(self, labels):
        ...
        if self.contains_spatial:
            ...
            if len(cls):
                ...
                an_new = self.transform(image=an_img, bboxes=bboxes, class_labels=cls)  # transformed
                if len(new["class_labels"]) > 0:  # skip update if no bbox in new im
                    labels["img"] = new["image"]
                    labels["an_img"] = an_new["image"]
...
class Format:
    ...
    def __call__(self, labels):
        ...
        if self.return_mask:
            ...
            labels["masks"] = masks
        labels["img"], labels["an_img"] = self._format_img(img, an_img)
    ...
    def _format_img(self, img, an_img):
        """
        ...
        if len(an_img.shape) < 3:
            an_img = np.expand_dims(an_img, -1)
        an_img = an_img.transpose(2, 0, 1)
        an_img = np.ascontiguousarray(an_img[::-1] if random.uniform(0, 1) > self.bgr else an_img)
        an_img = torch.from_numpy(an_img)
        return img, an_img
...
def v8_transforms1(dataset, imgsz, hyp, stretch=False):
    """
    Applies a series of image transformations for training.
