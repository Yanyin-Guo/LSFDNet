...
class BaseModel():
    ...
    def iter_setup_schedulers(self):
        ...
        if scheduler_type in ['MultiStepLR', 'MultiStepRestartLR']:
            ...
            for optimizer in self.optimizers:
                if cnt == 0:
                    self.schedulers.append(lr_scheduler.PolyLR(optimizer, train_opt['scheduler'], g_flag=True))
                else:
                    self.schedulers.append(lr_scheduler.PolyLR(optimizer, train_opt['scheduler'], g_flag=False))
                cnt += 1
        else:
            raise NotImplementedError(f'Scheduler {scheduler_type} is not implemented yet.')
